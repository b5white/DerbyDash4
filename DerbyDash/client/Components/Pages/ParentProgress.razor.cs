...
namespace DerbyDash.Components.Pages {
    public partial class ParentProgress : ComponentBase {
        ...
        [Inject] private IRaceService RaceService { get; set; } = default!;
        ...
        private async Task LoadData() {
            ...
            // Calculate statistics for each racer using IRaceService
            foreach (var racer in Racers) {
                try {
                    // Get race history for this racer
                    var raceHistory = await RaceService.GetRaceHistoryAsync(page: 1, pageSize: 1000, racerId: racer.Id);
                    ...
                    if (races.Any()) {
                        ...
                        RecentRaces[racer.Id] = races.OrderByDescending(r => r.RaceDateTime).Take(5).ToList();
                    }
                } catch (Exception ex) {
                    Logger.LogError(ex, "Error loading race data for racer {RacerId}", racer.Id);
                }
            }
